...
const canvas = document.querySelector("#test024Canvas");
...
function init() {
  ...
  for (i = 0; i < poslist.length; i++) {
    ...
    vertex.y = simplexNoise.noise(vertex.x / 2, vertex.z / 2);
  }
  ...
  // mesh = new THREE.Mesh(this.kokeModel2(poslist[40], random), material);
  // mesh.material.side = THREE.DoubleSide;
  // scene.add(mesh);
  ...
  spotlight = new THREE.SpotLight(0xffffff, 0.45, 30, Math.PI / 4, 10, 0.5);
...
function kokeModel2(baseLocation, random) {
  ...
  let bladeWid = 2;
  let v2d = bladeWid / 2;
  let vmhight = baseLocation.y + 1.08;
  let vthight = baseLocation.y + 0.2;
  ...
  //mid
  kokegeo.vertices.push(
    new THREE.Vector3(cosAM.x + p.x, vmhight, sinAM.x + p.y + 1.8),
  );
  kokegeo.vertices.push(
    new THREE.Vector3(cosAM.y + p.x, vmhight, sinAM.y + p.y + 1.0),
  );
  //top
  kokegeo.vertices.push(
    new THREE.Vector3(cosAT.x + p.x, vthight, sinAT.x + p.y + 3.0),
  );
  kokegeo.vertices.push(
    new THREE.Vector3(cosAT.y + p.x, vthight, sinAT.y + p.y + 3.5),
  );
