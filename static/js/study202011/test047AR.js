...
function init() {
  ...
  var kokemitu = 20;
  var kokemargin = new THREE.Vector2(-0.5, -0.5);
  var radius = 20;
  ...
  for (i = 0; i < poslist.length; i++) {
    ...
    vertex.y = simplexNoise.noise(vertex.x / 1, vertex.z / 1);
  }
...
function kokeModel2(baseLocation, random) {
  ...
  let bladeWid = 2 * 0.1;
  let v2d = bladeWid / 2;
  let vmhight = (baseLocation.y + 1.8) * 0.1;
  let vthight = (baseLocation.y + 3.5) * 0.1;
...
function kokeModel3(baseLocation, random) {
  ...
  let hight = (baseLocation.y + 2.5) * 0.1;
  let angle = random * Math.PI; //0.5 90
  let angleM = (random + 0.3) * Math.PI;
  let angleT = (random - 0.1 * random) * Math.PI;
